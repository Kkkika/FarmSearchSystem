      ...
      <div class="create-batch-container">
        ...
        <div class="form-content">
          ...
          >
            ...
            <!-- 养殖企业区域选择 -->
            <a-form-item label="养殖企业区域" required>
            ...
            <!-- 养殖企业选择 -->
            <a-form-item label="养殖企业" name="farmId" required>
              <a-select
                  v-model:value="formState.farmId"
                  placeholder="请选择养殖企业"
                  :options="farmOptions"
            ...
            <!-- 养殖企业产品批号 -->
            <a-form-item label="养殖企业产品批号" name="fbId" required>
              <a-select
                  v-model:value="formState.fbId"
                  placeholder="请选择养殖企业产品批号"
                  :options="farmBatchOptions"
                  :disabled="!formState.farmId"
                  allow-clear
                  show-search
            ...
            <!-- 检验检疫合格证编号 -->
            <a-form-item label="检验检疫合格证编号" name="quaId" required>
              <a-input
                  v-model:value="formState.quaId"
                  placeholder="请输入检验检疫合格证编号"
                  :maxlength="100"
                  allow-clear
              />
            </a-form-item>
            ...
            <!-- 检验员名称 -->
            <a-form-item label="检验员名称" name="testName" required>
              <a-input
...
import { createSlauBatch, getFarmsByArea, getFarmBatchesByFarm } from '@/api/slauBatchController'
import type { CreateSlauBatchRequestDto, BaseResponseInteger, FarmDto, FarmBatchSimpleDto } from '@/types/api.ts'
...
const farmOptions = ref<{ value: number; label: string }[]>([])
const farmBatchOptions = ref<{ value: number; label: string; batchId: string; type: string }[]>([])
...
const rules: Record<string, Rule[]> = {
  ...
  farmId: [
    { required: true, message: '请选择养殖企业', trigger: 'change' }
  ],
  fbId: [
    { required: true, message: '请选择养殖企业产品批号', trigger: 'change' }
  ],
  quaId: [
    { required: true, message: '请输入检验检疫合格证编号', trigger: 'blur' },
    { max: 100, message: '检验检疫合格证编号不能超过100个字符', trigger: 'blur' }
  ],
  testName: [
    { required: true, message: '请输入检验员名称', trigger: 'blur' },
    { max: 50, message: '检验员名称不能超过50个字符', trigger: 'blur' }
  ],
  remarks: [
...
const handleProvinceChange = async (value: number) => {
  ...
  formState.farmId = undefined
  formState.fbId = undefined
  cityOptions.value = []
  farmOptions.value = []
  farmBatchOptions.value = []
...
const handleCityChange = async (value: number) => {
  ...
  if (value && formState.provId) {
    await loadFarmsByArea()
  }
}

// 根据区域加载养殖企业
const loadFarmsByArea = async () => {
  if (!formState.provId || !formState.cityId) return

  try {
    loadingFarms.value = true
    const response = await getFarmsByArea({
      provId: formState.provId,
      cityId: formState.cityId
    })

    if (response.code === 0 && response.data) {
      farmOptions.value = response.data.map(farm => ({
        value: farm.nodeId!,
        label: farm.name!
      }))
    } else {
      message.error(response.message || '加载养殖企业失败')
    }
  } catch (error) {
    console.error('加载养殖企业失败:', error)
    message.error('加载养殖企业失败')
  } finally {
...
// 养殖企业选择变化
const handleFarmChange = async (value: number) => {
  formState.fbId = undefined
  farmBatchOptions.value = []

  if (value) {
    await loadFarmBatches(value)
  }
}

// 加载养殖企业产品批号
const loadFarmBatches = async (farmId: number) => {
  try {
    loadingBatches.value = true
    const response = await getFarmBatchesByFarm({ farmId })

    if (response.code === 0 && response.data) {
      farmBatchOptions.value = response.data.map(batch => ({
        value: batch.fbId!,
      ...
      message.error(response.message || '加载养殖企业产品批号失败')
    }
  } catch (error) {
    console.error('加载养殖企业产品批号失败:', error)
    message.error('加载养殖企业产品批号失败')
  } finally {
...
// 养殖企业搜索过滤
const filterFarmOption = (input: string, option: any) => {
...
const handleSubmit = async () => {
  try {
    ...
    const submitData: CreateSlauBatchRequestDto = {
      batchId: formState.batchId.trim(),
      slauId: authStore.userInfo?.userId!, // 使用当前登录用户的ID作为屠宰企业ID
      fbId: formState.fbId!,
      type: formState.type!,
      quaId: formState.quaId,
      testName: formState.testName,
      remarks: formState.remarks || undefined
    }

    const response: BaseResponseInteger = await createSlauBatch(submitData)
