      ...
      <div class="create-batch-container">
        ...
        <div class="form-content">
          ...
          >
            ...
            <!-- 屠宰企业区域选择 -->
            <a-form-item label="屠宰企业区域" required>
            ...
            <!-- 屠宰企业选择 -->
            <a-form-item label="屠宰企业" name="slauId" required>
              <a-select
                  v-model:value="formState.slauId"
                  placeholder="请选择屠宰企业"
                  :options="slaughterhouseOptions"
                  :disabled="!formState.cityId"
                  @change="handleSlaughterhouseChange"
                  allow-clear
                  show-search
                  :filter-option="filterSlaughterhouseOption"
                  :loading="loadingSlaughterhouses"
              />
            </a-form-item>

            <!-- 屠宰企业产品批号 -->
            <a-form-item label="屠宰企业产品批号" name="sbId" required>
              <a-select
                  v-model:value="formState.sbId"
                  placeholder="请选择屠宰企业产品批号"
                  :options="slauBatchOptions"
                  :disabled="!formState.slauId"
                  allow-clear
                  show-search
...
import {
  createWholBatch,
  getSlaughterhousesByArea,
  getSlauBatchesBySlaughterhouse
} from '@/api/wholBatchController'
import type {
  CreateWholBatchRequestDto,
  BaseResponseInteger,
  SlaughterhouseDto,
  SlauBatchSimpleDto
} from '@/types/api.ts'
...
const formState = reactive({
  ...
  slauId: undefined as number | undefined,
  sbId: undefined as number | undefined,
  remarks: ''
})
...
const loadingSlaughterhouses = ref(false)
const loadingBatches = ref(false)

const provinceOptions = ref<{ value: number; label: string }[]>([])
const cityOptions = ref<{ value: number; label: string }[]>([])
const slaughterhouseOptions = ref<{ value: number; label: string }[]>([])
const slauBatchOptions = ref<{ value: number; label: string; batchId: string; type: string }[]>([])
...
const rules: Record<string, Rule[]> = {
  ...
  sbId: [
    { required: true, message: '请选择屠宰企业产品批号', trigger: 'change' }
  ],
  remarks: [
...
const handleProvinceChange = async (value: number) => {
  ...
  formState.slauId = undefined
  formState.sbId = undefined
  cityOptions.value = []
  slaughterhouseOptions.value = []
  slauBatchOptions.value = []
...
const handleCityChange = async (value: number) => {
  ...
  if (value && formState.provId) {
    await loadSlaughterhousesByArea()
  }
}

// 根据区域加载屠宰企业
const loadSlaughterhousesByArea = async () => {
  if (!formState.provId || !formState.cityId) return

  try {
    loadingSlaughterhouses.value = true
    const response = await getSlaughterhousesByArea({
      provId: formState.provId,
      cityId: formState.cityId
    })

    if (response.code === 0 && response.data) {
      slaughterhouseOptions.value = response.data.map(slaughterhouse => ({
        value: slaughterhouse.nodeId!,
        label: slaughterhouse.name!
      }))
    } else {
      message.error(response.message || '加载屠宰企业失败')
    }
  } catch (error) {
    console.error('加载屠宰企业失败:', error)
    message.error('加载屠宰企业失败')
  } finally {
    loadingSlaughterhouses.value = false
  }
}

// 屠宰企业选择变化
const handleSlaughterhouseChange = async (value: number) => {
  formState.sbId = undefined
  slauBatchOptions.value = []

  if (value) {
    await loadSlauBatches(value)
  }
}

// 加载屠宰企业产品批号
const loadSlauBatches = async (slauId: number) => {
  try {
    loadingBatches.value = true
    const response = await getSlauBatchesBySlaughterhouse({ slauId })

    if (response.code === 0 && response.data) {
      slauBatchOptions.value = response.data.map(batch => ({
        value: batch.sbId!,
      ...
      message.error(response.message || '加载屠宰企业产品批号失败')
    }
  } catch (error) {
    console.error('加载屠宰企业产品批号失败:', error)
    message.error('加载屠宰企业产品批号失败')
  } finally {
...
// 屠宰企业搜索过滤
const filterSlaughterhouseOption = (input: string, option: any) => {
...
const handleSubmit = async () => {
  try {
    ...
    const submitData: CreateWholBatchRequestDto = {
      batchId: formState.batchId.trim(),
      wholId: authStore.userInfo?.userId!, // 使用当前登录用户的ID作为批发商ID
      sbId: formState.sbId!,
      type: formState.type!,
      remarks: formState.remarks || undefined
    }

    const response: BaseResponseInteger = await createWholBatch(submitData)
